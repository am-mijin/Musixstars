//
//  AVPlayerDemoPlaybackViewController.m
//
...
@synthesize mPlayer, mPlayerItem, mPlaybackView, mScrubber;
...
- (IBAction)play:(id)sender
{
    if ([self isPlaying])
    {
        ...
        [_mPlayButton setImage:[UIImage imageNamed:@"play_button"] forState:UIControlStateNormal];
        [_mPlayButton setImage:[UIImage imageNamed:@"play_button"] forState:UIControlStateSelected];
        [self.mPlayer pause];
    }
    else
    {
        
        [_mPlayButton setImage:[UIImage imageNamed:@"big_pause"] forState:UIControlStateNormal];
        [_mPlayButton setImage:[UIImage imageNamed:@"big_pause"] forState:UIControlStateSelected];
        [self.mPlayer play];
    }
...
- (void)syncScrubber
{
    ...
    _mDuration.text = [NSString stringWithFormat:@"%d : %d",(int)duration/60,(int)duration %60];
    
    
    if ((int)duration/60 <10)
    {
        minstr= [NSString stringWithFormat:@"0%d",(int)duration/60];
    }
    else
        minstr= [NSString stringWithFormat:@"%d",(int)duration/60];
    
    if ((int)duration%60 <10)
        secstr= [NSString stringWithFormat:@"0%d",(int)duration%60];
    
    else
        secstr= [NSString stringWithFormat:@"%d",(int)duration%60];
    
    _mDuration.text = [NSString stringWithFormat:@"%@ : %@",minstr,secstr];
    ...
    if (isfinite(duration))
    {
        ...
        if ((int)time/60 <10)
        {
            minstr= [NSString stringWithFormat:@"0%d",(int)time/60];
        }
        else
            minstr= [NSString stringWithFormat:@"%d",(int)time/60];
        
        if ((int)time%60 <10)
            secstr= [NSString stringWithFormat:@"0%d",(int)time%60];
        
        else
            secstr= [NSString stringWithFormat:@"%d",(int)time%60];
        
        _mTime.text = [NSString stringWithFormat:@"%@ : %@",minstr,secstr];
        
        //NSLog(@"%d:%d, %d",(int)min, (int)duration%60, Seconds %60);
...
{
    [_mPlayButton setHidden:NO ];
    [mScrubber setHidden:NO ];
    [_mDuration setHidden:NO];
    [_mTime setHidden:NO];
    [_mNaviBar setHidden:NO];
        
    
    if( !_activityIndicatorView.hidden)
    {
        [mScrubber setHidden:YES ];
        [_mDuration setHidden:YES];
        [_mTime setHidden:YES];
        [_mPlayButton setHidden:YES ];
        
    }
    
    //    [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:UIStatusBarAnimationNone];
    [self performSelector:@selector(hideOverlayView) withObject:nil afterDelay:3.5f];
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([touch view] == mPlaybackView)
    {
        
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideOverlayView) object: nil];
...
- (IBAction)scrub:(id)sender
{
    if ([sender isKindOfClass:[UISlider class]])
    {
        ...
        if (isfinite(duration))
        {
            ...
            //_mTime.text = [NSString stringWithFormat:@"%d : %d",(int)time/60,(int)time %60];
...
- (IBAction)playVideo:(id)sender
{
    ...
    //[self performSelector:@selector(hideOverlayView) withObject:nil afterDelay:3.5f];
    //http://cdn.ilovestage.co.uk/videos/39steps-epk-final-h264.mov
    //http://cdn.ilovestage.co.uk/videos/cats-1-min-hd-version-40k-1920.mov
    //http://cdn.ilovestage.co.uk/videos/encore-mp4-720x404-mst-l11766-memphis-60-v2-01.mp4
    //http://cdn.ilovestage.co.uk/videos/once-sizzle-reel-final-london.mp4
    //http://cdn.ilovestage.co.uk/videos/online-mstl11542-warhorse-epk-edit-ud.mov
    //http://cdn.ilovestage.co.uk/videos/shakespeare-in-love-trailer-no-url.mp4
    //
    [self setURL:[NSURL URLWithString:@"http://cdn.ilovestage.co.uk/videos/shakespeare-in-love-trailer-no-url.mp4"]];
}
...
- (void)viewDidLoad
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(stopVideoPlayer:) name:NOTIFICATION_STOP_VIDEOPLAYER object:nil];
    
    [super viewDidLoad];
    
}

-(void)stopVideoPlayer:(NSNotification*)notification
{
    [self.mPlayer pause];
}
//-(void)setup
...
- (CMTime)playerItemDuration
{
    ...
    if (playerItem.status == AVPlayerItemStatusReadyToPlay)
    {
        ...
        //NSLog(@"%f",[[[[[playerItem tracks] objectAtIndex:0] assetTrack] asset] duration]);
