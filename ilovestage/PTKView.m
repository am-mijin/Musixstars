...
- (void)setupCardNumberField
{
    ...
    if([[Global sharedInstance].card.number length])
    {
        ...
        self.cardNumberField.placeholder = [NSString stringWithFormat:@"···· ···· ···· %@",trimmedString];

    }
    else{
        self.cardNumberField.placeholder = @"Card Number";
    }
    ...
    self.cardNumberField.font =  [UIFont fontWithName:@"Seravek" size:16];
    [self.cardNumberField.layer setMasksToBounds:NO];
}
...
- (void)setupCardExpiryField
{
    ...
    self.cardExpiryField.font =  [UIFont fontWithName:@"Seravek" size:16];
